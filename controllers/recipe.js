...
const getAllRecipesWithPagination = async (req, res) => {
    ...
    await pool.query(command, (err, result) => {
        if(err) res.json({error: err})
            ...
            const postsOnPage = 6
            let allRecipes = [...result.rows]
            allRecipes.sort((a, b) => b.created - a.created)
...
const deleteRecipe = async (req, res) => {
    try {
        const {token, recipe} = req.body
        const id = jwt.verify(token, process.env.JWT_ACCESS_SECRET)
        const command = `delete from recipes where dish_name = '${recipe}' and author_email = '${id.email}'`
        await pool.query(command, (err, result) => {
            if(err) res.json({error: err})
...
module.exports = {newRecipe, getRecipes, uploadImage, getMyRecipes, getChefs, getOneRecipe, addToFavorites, deleteFromFavorites, rateRecipe, checkRating, checkFavorites, getFavorites, getAllRecipesWithPagination, updateRecipe, deleteRecipe}